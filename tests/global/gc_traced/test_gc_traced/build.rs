...
fn main() {
    println!("cargo:rerun-if-changed=src/test_gc_traced.ridl");
...
fn resolve_ridl_tool() -> PathBuf {
    env::var("MQUICKJS_RIDL_TOOL")
        .map(PathBuf::from)
        .unwrap_or_else(|_| {
            panic!("MQUICKJS_RIDL_TOOL is not set. Hint: run `cargo run -p ridl-builder -- prepare` first.")
        })
}
