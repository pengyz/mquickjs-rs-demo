...
pub fn generate_shared_files(
    ...
    for ridl_file in ridl_files {
        ...
        for item in items {
            match item {
                ...
                crate::parser::ast::IDLItem::Singleton(s) => singletons.push(s),
                crate::parser::ast::IDLItem::Class(c) => {
                    classes.push(TemplateClass::from_with_mode(c, parsed.mode))
                }
                // 其他类型暂不处理
                _ => {}
            }
        ...
        all_module_symbols.push((module_name, functions, interfaces, classes, singletons));
    }
    ...
    for (_, functions, interfaces, classes, singletons) in &all_module_symbols {
        all_functions.extend(functions.clone());
        all_interfaces.extend(interfaces.clone());
        all_classes.extend(classes.clone());
    ...
    let c_code = ridl_register_h.render()?;
    std::fs::write(
        std::path::Path::new(output_dir).join("mquickjs_ridl_register.h"),
        c_code,
    )?;
