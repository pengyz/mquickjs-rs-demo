...
impl Context {
    ...
    pub fn eval(&mut self, code: &str) -> Result<String, String> {
        ...
        if tag == mquickjs_ffi::JS_TAG_EXCEPTION as u32 {
            ...
            let error_ptr = unsafe { 
                mquickjs_ffi::JS_ToCString(self.ctx, exception, &mut cstr_buf) 
            };
            ...
            let result_ptr = unsafe { 
                mquickjs_ffi::JS_ToCString(self.ctx, result, &mut cstr_buf) 
            };
    ...
    pub fn create_string(&self, rust_str: &str) -> Result<Value, String> {
        ...
        let js_value = unsafe {
            mquickjs_ffi::JS_NewString(self.ctx, c_str.as_ptr())
        };
        
        if (js_value as u32) & ((1 << mquickjs_ffi::JS_TAG_SPECIAL_BITS) - 1) == mquickjs_ffi::JS_TAG_EXCEPTION as u32 {
    ...
    pub fn create_number(&self, num: f64) -> Result<Value, String> {
        let js_value = unsafe {
            mquickjs_ffi::JS_NewFloat64(self.ctx, num)
        };
    ...
    pub fn create_object(&self) -> Result<Value, String> {
        ...
        if (obj as u32) & ((1 << mquickjs_ffi::JS_TAG_SPECIAL_BITS) - 1) == mquickjs_ffi::JS_TAG_EXCEPTION as u32 {
    ...
    pub fn get_string(&self, value: Value) -> Result<String, String> {
        ...
        let mut cstr_buf = mquickjs_ffi::JSCStringBuf { buf: [0; 5] };
        let result_ptr = unsafe { 
            mquickjs_ffi::JS_ToCString(self.ctx, value.value, &mut cstr_buf) 
        };
    ...
    pub fn get_number(&self, value: Value) -> Result<f64, String> {
        ...
        let mut result = 0.0;
        let ret = unsafe {
            mquickjs_ffi::JS_ToNumber(self.ctx, &mut result, value.value)
        };
    ...
    pub fn get_boolean(&self, value: Value) -> Result<bool, String> {
        ...
        let mut result = 0i32;
        let ret = unsafe {
            mquickjs_ffi::JS_ToInt32(self.ctx, &mut result, value.value)
        };
