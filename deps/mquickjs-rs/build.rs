...
fn main() {
    ...
    generate_js_class_id_rs(&out_path, &include_dir);
...
fn generate_js_class_id_rs(out_dir: &Path, include_dir: &Path) {
    // Source of truth for JS_CLASS_* allocation is the aggregate header generated by ridl-builder.
    // Example: #define JS_CLASS_TEST_REQUIRE_FOO (JS_CLASS_USER + 2)
    let in_path = include_dir.join("mquickjs_ridl_api.h");
    println!("cargo:rerun-if-changed={}", in_path.display());

    let Ok(content) = fs::read_to_string(&in_path) else {
        // If the header does not exist (e.g. before ridl-builder prepare), keep an empty file.
        let _ = fs::write(out_dir.join("ridl_js_class_id.rs"), "\n");
        return;
    };

    let items = parse_js_class_id_defines(&content);
    let mut out = String::new();
    out.push_str("// @generated by mquickjs-rs/build.rs. DO NOT EDIT.\n\n");
    for (name, value) in items {
        out.push_str(&format!(
    ...
    fs::write(out_dir.join("ridl_js_class_id.rs"), out).expect("write ridl_js_class_id.rs");
}

fn parse_js_class_id_defines(h: &str) -> Vec<(String, i32)> {
    // Minimal parser for:
    //   #define JS_CLASS_FOO (JS_CLASS_USER + 2)
    // We only care about JS_CLASS_USER-relative defines.
    let mut out = Vec::new();
    ...
    for raw in h.lines() {
        ...
        // Split: #define NAME (JS_CLASS_USER + N)
        let mut it = line.split_whitespace();
        let _define = it.next();
        let Some(name) = it.next() else { continue };
        let Some(rest0) = it.next() else { continue };
        let rest = std::iter::once(rest0).chain(it).collect::<Vec<_>>().join(" ");
        let rest = rest.trim();

        // Expect: (JS_CLASS_USER + <int>)
        let Some(inner) = rest.strip_prefix('(').and_then(|s| s.strip_suffix(')')) else {
            continue;
        };
        let inner = inner.trim();
        if !inner.starts_with("JS_CLASS_USER") {
            continue;
        }
        let Some((_, rhs)) = inner.split_once('+') else {
            continue;
        };
        let rhs = rhs.trim();
        if let Ok(v) = rhs.parse::<i32>() {
